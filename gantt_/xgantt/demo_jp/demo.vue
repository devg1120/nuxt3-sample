<template>
  <div style="top: 20vh; width: 1400px; height: 500px; padding-left: 5vw">
    ...
    >
      ...
      <x-gantt-column v-slot="scope" label="開始日">
  ...
  <div>
    合{{ ganttData.length }}計
    <button @click="onAdd"> + </button>
    <button @click="onReduce"> - </button>
    <button @click="onEmpty">クリア</button>
    <button @click="onReload">再割り当て</button>
    <button @click="() => (draggable = !draggable)">ドラッグ</button>
    <button @click="onExpand">{{ showExpand ? 'Hide' : 'Show' }}</button>
    <button @click="onExpandAll">
      {{ expandAll ? '閉鎖' : '拡大' }}
    </button>
    <button @click="onChangeBorderColor">border-color</button>
    <button @click="changeUnit">changeUnit</button>
    <button @click="jumpToDate">jumpToDate</button>
    <button @click="setSelected">setSelected</button>
    <button @click="setDark">{{ isDark ? 'white' : 'Dark' }}</button>
  </div>
